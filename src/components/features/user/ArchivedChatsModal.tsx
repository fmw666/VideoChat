...
export const ArchivedChatsModal: FC<ArchivedChatsModalProps> = ({
  isOpen,
  onClose,
}) => {
  // --- State and Refs ---
  ...
  const [showDeleteAllArchivedConfirm, setShowDeleteAllArchivedConfirm] =
    useState(false);
  ...
  return (
    <>
          ...
          >
            <div>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {t('settings.data.archivedChatsCount', {
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="inline-flex items-center gap-2 px-3 py-2 text-sm font-medium text-zinc-700 dark:text-zinc-300 bg-zinc-100 dark:bg-zinc-800/50 rounded-lg hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isUnarchivingAll ? (
                  <>
              ...
              <AnimatePresence mode="popLayout">
                {sortedGroupKeys.map((groupKey, groupIndex) => (
                  ...
                  >
                    ...
                    >
                      <div className="w-1 h-4 bg-gradient-to-b from-zinc-500 to-zinc-600 rounded-full"></div>
                      <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">
                        {groupKey}
                      </h3>
                      <span className="text-xs text-gray-500 dark:text-gray-400">
                        ({groupedArchivedChats[groupKey].length}{' '}
                        {t('history.item')})
                      </span>
                    </motion.div>
                    ...
                    <div className="space-y-2 pl-3">
                      {groupedArchivedChats[groupKey].map((chat, chatIndex) => {
                        const isItemLoading =
                          isDeleting === chat.id || isUnarchiving === chat.id;
                        ...
                        return (
                          <motion.div
                            ...
                            transition={{
                              duration: 0.4,
                              delay:
                                groupIndex * 0.08 + chatIndex * 0.03 + 0.15,
                              ease: [0.25, 0.46, 0.45, 0.94],
                            }}
                            className="group"
                          >
                            <div
                              className={`flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg transition-all duration-200 ${
                            ...
                            >
                                ...
                                <div
                                  ...
                                  className={`font-medium text-gray-900 dark:text-white truncate transition-colors duration-200 ${
                                    isGlobalLoading
                                      ...
                                      : 'hover:text-zinc-700 dark:hover:text-zinc-300 cursor-pointer'
                                  }`}
                                  title={
                                    isGlobalLoading
                                      ? ''
                                      : 'Click to open chat (Ctrl+Click for new tab)'
                                  }
                                >
                                  {chat.title}
                                </div>
                              ...
                              <div className="flex items-center gap-2 flex-shrink-0">
                                <button
                                  ...
                                  className="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium text-zinc-700 dark:text-zinc-300 bg-zinc-100 dark:bg-zinc-800/50 rounded-md hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                                >
      ...
      <ConfirmDialog
        ...
        onClose={
          isGlobalLoading ? () => {} : () => setShowUnarchiveAllConfirm(false)
        }
