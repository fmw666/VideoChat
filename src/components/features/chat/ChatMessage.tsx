...
const MessageParamsDisplay: FC<{
  ...
  const hasConfig =
    message.videoConfig &&
    Object.keys(message.videoConfig).some(
        ...
        {isExpanded && (
          <div className="w-full max-w-md bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-3 text-left shadow-sm">
              ...
              <div className="mb-3">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {message.models.map((model, index) => (
                    <span
                      key={`${model.id}-${index}`}
                      className="inline-flex items-center gap-1 px-2 py-0.5 text-xs bg-zinc-100 dark:bg-zinc-800/50 text-zinc-700 dark:text-zinc-300 rounded-full"
                    >
              ...
              <div className="mb-3">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                  ...
                  {message.videoConfig.enhanceSwitch && (
                    <div className="flex justify-between">
                      <span className="text-gray-500 dark:text-gray-400">
                        {t('chat.params.enhanceSwitch', '画质增强')}
                      </span>
                      <span
                        className={`${message.videoConfig.enhanceSwitch === 'Enabled' ? 'text-green-600 dark:text-green-400' : 'text-gray-400'}`}
                      >
                        {message.videoConfig.enhanceSwitch === 'Enabled'
                          ? t('common.enabled', '开启')
                          : t('common.disabled', '关闭')}
                      </span>
                    </div>
                  )}
                  {message.videoConfig.audioGeneration && (
                    <div className="flex justify-between">
                      <span className="text-gray-500 dark:text-gray-400">
                        {t('chat.params.audioGeneration', '音频生成')}
                      </span>
                      <span
                        className={`${message.videoConfig.audioGeneration === 'Enabled' ? 'text-green-600 dark:text-green-400' : 'text-gray-400'}`}
                      >
                        {message.videoConfig.audioGeneration === 'Enabled'
                          ? t('common.enabled', '开启')
                          : t('common.disabled', '关闭')}
                      </span>
                    </div>
                ...
                {message.videoConfig.negativePrompt && (
                  <div className="mt-1.5 text-xs">
                    <span className="text-gray-500 dark:text-gray-400">
                      {t('chat.params.negativePrompt', '负面提示词')}:{' '}
                    </span>
            ...
            {(() => {
              ...
              const hasNewFormat =
                message.videoConfig?.modelSpecificParams &&
                message.videoConfig.modelSpecificParams.length > 0;
              ...
              return (
                <div className="mb-3">
                  ...
                  <div className="space-y-2">
                    {/* 新格式的模型专属参数 */}
                    {hasNewFormat &&
                      message.videoConfig!.modelSpecificParams!.map(
                        (modelParam, idx) => (
                          <div
                          ...
                          >
                            ...
                            <div className="text-xs space-y-0.5">
                              {Object.entries(modelParam.params).map(
                                ([key, value]) => (
                                  <div
                                    key={key}
                                  ...
                                  >
                                    ...
                                    <span className="text-gray-700 dark:text-gray-300">
                                      {key === 'sceneType'
                                        ? value === 'default' || value === ''
                                          ? t(
                                              'chat.params.sceneTypeDefault',
                                              '默认'
                                            )
                                          : value === 'motion_control'
                                            ? t(
                                                'chat.params.sceneTypeMotionControl',
                                                '动作控制'
                                              )
                                            : String(value)
                                        : String(value)}
                      ...
                      <div className="p-2 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800">
                        ...
                        <div className="text-xs space-y-0.5">
                          <div className="flex justify-between">
                            ...
                            <span className="text-gray-700 dark:text-gray-300">
                              {(message.videoConfig as any).sceneType}
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <>
                        {message.uploadedImages.map((img, index) => (
                          <div
                            ...
                            title={t(
                              'chat.params.clickToEnlarge',
                              '点击放大查看'
                            )}
                          >
                            <div className="w-14 h-14 rounded-lg overflow-hidden border border-zinc-200 dark:border-zinc-700 bg-gray-100 dark:bg-gray-800">
                              <img
                                src={img.url}
                                alt={img.name}
                            ...
                            <span className="absolute -top-1 -left-1 w-4 h-4 bg-zinc-700 dark:bg-zinc-300 text-white dark:text-zinc-800 text-[10px] rounded-full flex items-center justify-center">
                              {index === 0
                                ? t('chat.params.firstFrame', '首')
                                : index + 1}
                            </span>
                            {/* 放大图标提示 */}
                  ...
                  {message.lastFrameImage && (
                    <>
                      ...
                      <div
                        ...
                        onClick={() =>
                          handleImageClick(
                            message.lastFrameImage!.url,
                            message.lastFrameImage!.name
                          )
                        }
...
export const ChatMessage: FC<ChatMessageProps> = ({
  ...
  const handleJumpToReference = useCallback(
    (messageId: string, resultId: string) => {
        ...
        <div className="flex justify-end p-3">
          <div className="flex items-start gap-3 max-w-3xl">
            <div className="flex-1 text-right">
              <div className="inline-block bg-zinc-800 dark:bg-zinc-200 text-white dark:text-zinc-800 text-left rounded-lg px-4 py-2">
                  ...
                  <div
                    ...
                    onClick={() =>
                      onEnterDesign?.({
                        ...
                        messageId:
                          message.userImage?.referenceMessageId || null,
            ...
            <div className="flex-shrink-0">
              ...
              >
                <div
                  className={`${getAvatarClasses()} ${getAvatarSizeClasses('sm')}`}
                >
          ...
          <div className={'flex items-start gap-3 max-w-8xl'}>
            ...
            <div className="flex-1">
                    ...
                    <>
                      ...
                      {isHiddenModelInfo && (
                        <span className="ml-2 text-xs text-gray-500">
                          ({Object.values(message.results.videos).flat().length}{' '}
                          {t('chat.videos')})
                        </span>
                      )}
                    </>
              ...
              {isHiddenModelInfo ? (
                ...
                // 显示模型信息模式下的视频展示
                Object.entries(message.results.videos).map(
                  ([modelId, results], index) => (
                    <div key={modelId} className="mb-4">
                      {!message.userImage && (
                        <div className="flex items-center gap-2 mb-2">
                          <div className="h-5 w-5 rounded-full bg-zinc-200 dark:bg-zinc-600 flex items-center justify-center shadow-sm">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-3 min-w-[300px]">
                        {results.map(
                          (result: VideoResult, resultIndex: number) => (
                            <VideoResultItem
