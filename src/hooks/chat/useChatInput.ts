...
import type { Chat, Message, Results, VideoResult, MessageVideoConfig, MessageUploadedImage, ModelSpecificParams } from '@/services/chat';
...
import { isModelAvailableForInput, canUseLastFrame } from '@/config/models.types';
// --- Utils ---
...
export const useChatInput = ({
  ...
  const handleSendMessage = useCallback(async (e: FormEvent) => {
    ...
    try {
      ...
      // 上传图片到云存储的辅助函数
      const uploadImageToCloud = async (img: UploadedImage, prefix: string = 'i2v'): Promise<string> => {
        ...
        if (img.file) {
          ...
          const result = await storageService.uploadImage(img.file, safeFileName);
      ...
      try {
        ...
        if (uploadedImages.length > 0) {
          console.log(`准备上传 ${uploadedImages.length} 张首帧图片到云存储...`);
          const uploadedUrls = await Promise.all(
            uploadedImages.map(img => uploadImageToCloud(img, 'first'))
          );
          sharedFileInfos = uploadedUrls.map(url => ({ Type: 'Url' as const, Url: url }));
        ...
        if (lastFrameImage) {
          ...
          sharedLastFrameUrl = await uploadImageToCloud(lastFrameImage, 'last');
          lastFrameImageInfo = {
      ...
      const initialResults: Results = {
        ...
        videos: currentModels.reduce((acc, model) => ({
          ...acc,
          [model.name]: Array(model.count).fill(null).map((_, index) => createInitialVideoResult(index))
        }), {})
      };
      ...
      const messageVideoConfig: MessageVideoConfig = {
        ...
        modelSpecificParams: modelSpecificParams.length > 0 ? modelSpecificParams : undefined,
      };
      ...
      try {
        ...
        currentModels.forEach(({ name, count }) => {
          ...
          const errorResults: VideoResult[] = Array(count).fill(null).map((_, index) => ({
      ...
      const updatePromises = currentModels.map(async ({ id, name, count }) => {
        ...
        if (!model) {
          ...
          onUpdateMessageResults(message.id, { ...message.results, videos: updatedVideos }, true);
        ...
        const compatibility = isModelAvailableForInput(model, hasUploadedImages, imageCount);
        if (!compatibility.available) {
          ...
          if (compatibility.reason === 'onlyT2V') {
            warningMessage = t('model.incompatible.onlyT2V', '该模型仅支持文生视频，不支持图片输入');
            shortError = t('model.incompatible.shortOnlyT2V', '不支持图片输入');
          } else if (compatibility.reason === 'onlyI2V') {
            warningMessage = t('model.incompatible.onlyI2V', '该模型仅支持图生视频，需要上传图片');
            shortError = t('model.incompatible.shortOnlyI2V', '需要上传图片');
          ...
          // 创建警告/错误结果
          const warningResults: VideoResult[] = Array(count).fill(null).map((_, index) => ({
        ...
        try {
          ...
          return new Promise<{ [key: string]: VideoResult[] }>((resolve) => {
            // 初始化结果数组
            const results: VideoResult[] = Array(count).fill(null).map((_, index) => 
              createInitialVideoResult(index)
            );

            const streamRequest = {
              count,
              onProgress: (update: VideoStatusUpdate, index: number, total: number) => {
                console.log(`视频生成进度: ${name} - ${index + 1}/${total}`, update);
                
                // 更新对应索引位置的结果
                results[index] = statusUpdateToVideoResult(update, results[index]);
              ...
              onError: (error: Error) => {
                ...
                // 计算未完成的数量
                const incompleteCount = results.filter(r => r.isGenerating).length;
            ...
            const lastFrameCheck = canUseLastFrame(model, resolution, firstFrameCount);
            ...
            const videoRequest = {
              ...
              negativePrompt: videoConfig.negativePrompt || undefined,
              // 尾帧图片（根据模型限制条件判断）
              lastFrameUrl: lastFrameCheck.canUse ? sharedLastFrameUrl : undefined,
              // Kling 特有配置
              sceneType: model.modelName === 'Kling' ? videoConfig.sceneType : undefined,
            };
  ...
  }, [user, selectedModels, designImage, uploadedImages, lastFrameImage, videoConfig, hasUploadedImages, currentChat, onCreateNewChat, onNavigate, onSendMessage, onUpdateMessageResults, onSetInput, onSetIsSending, onSetIsGenerating, onScrollToBottom, t]);
